...
// ini SD and create files
    ...
    File file = SD.open("/data.txt");// open file 
    File file2 = SD.open("/Backup.txt");// open file 
    if(!file) {
...
void printStringValues(){
  File file = SD.open("/data.txt");// open file 
  Serial.println("------------------------\n");  
  Serial.println("Reading the data......");  

    while (file.available()){
      String line = String(file.read());
      Serial.println(line);
      appendFile(SD, "/Backup.txt", line.c_str());
